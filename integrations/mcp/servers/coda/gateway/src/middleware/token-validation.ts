...
/**
 * Verify token with Coda API
 *
 * Validates bearer token by calling Coda's /v1/whoami endpoint
 */
export async function verifyToken(token: string): Promise<AuthInfo> {
  try {
    ...
    if (!response.ok) {
      return {
        valid: false,
        error: `Coda API error: ${response.status} ${response.statusText}`
      };
    }

    const data = await response.json() as any;

    // Extract user info from Coda API response
    const userId = data.user?.id || data.id || 'unknown';
