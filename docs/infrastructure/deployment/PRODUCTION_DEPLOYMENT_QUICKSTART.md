...
- source_path: /docs/infrastructure/deployment/PRODUCTION_DEPLOYMENT_QUICKSTART.md
- date: 2025-10-26
...
nano ../config/.env   # or use vim
```

Populate the following keys:
- `CF_TUNNEL_TOKEN`, `DOMAIN`, `LETSENCRYPT_EMAIL`
- `POSTGRES_PASSWORD`, `N8N_*`
- `CODA_API_TOKEN`, `DIGITALOCEAN_API_TOKEN`
- `CLOUDFLARE_REMOTE_URL`, `CLOUDFLARE_API_TOKEN` (if required)
- `QDRANT_API_KEY`

### Step 2: Create Data & Cert Directories

```bash
mkdir -p data/{postgres,qdrant,n8n,coda-mcp}
mkdir -p certs vhost.d html acme logs
chmod 755 data/* certs acme
```

Volumes in `infra/docker/` match the compose file. Adjust permissions if Docker needs different ownership.
