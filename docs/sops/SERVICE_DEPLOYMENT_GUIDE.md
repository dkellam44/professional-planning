...
> Note: some apps (e.g., Dozzle) only allow `GET` and will return `405 Method Not Allowed` to a bare `HEAD` probe. If you see that response, re-run the check with an explicit GET: `curl -sSf https://service.bestviable.com/ >/dev/null`.

---

### Proxy Stack Checklist (must be green before exposing services)

| Requirement | Why it matters | Command / Where to check |
|-------------|----------------|---------------------------|
| `nginx-proxy` + `acme-companion` mount `vhost` and `html` volumes | ACME HTTP-01 challenges need a shared writeable path | `grep -A2 '/etc/nginx/vhost.d' infra/n8n/docker-compose.yml` |
| Services set `VIRTUAL_HOST`, `VIRTUAL_PORT`, `LETSENCRYPT_HOST`, `LETSENCRYPT_EMAIL` **in environment** | jwilder registers upstreams from env vars (labels often ignored) | `docker inspect <service> --format '{{range .Config.Env}}{{println .}}{{end}}' | grep VIRTUAL_HOST` |
| Reverse proxy restarted after config changes | Regenerates nginx config, retries ACME | `docker compose -f infra/n8n/docker-compose.yml up -d` |
| Service compose redeployed after env edits | Ensures new metadata is visible to proxy | `docker compose -f infra/apps/<service>/docker-compose.yml up -d` |
| Cloudflare route added manually | Tunnel does not auto-create hostnames | Cloudflare Zero Trust → Access → Tunnels |

> Tip: keep this table open when onboarding a new service. 90% of tunnel issues (502/503, SSL errors, 404 webhooks) trace back to one of the items above.

---

### Cloudflare WAF Hardening (recommended)

Add a lightweight firewall rule to block anonymous `.git` harvesters and HEAD scanners directly at Cloudflare so the tunnel never sees them:

1. **Create an API token** with permissions `Zone.Firewall Services:Edit` for the `bestviable.com` zone.
2. **Apply the rule** (replace placeholders before running):
   ```bash
   CF_API_TOKEN=cfp_... \
   CF_ACCOUNT_TAG=xxxxxxxxxxxxxxxxxxxxx \
   CF_ZONE_ID=yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy \
     ...
     --data '[
       {
         "action": "block",
         "filter": {
           "expression": "(http.request.uri.path contains \".git\" and not cf.client.bot) or (http.request.method eq \"HEAD\" and http.request.headers[\"host\"][0] eq \"\")"
         },
         "description": "Block .git probes and anonymous HEAD scans"
       }
     ]'
   ```
3. **Verify** in Zero Trust → Security → Firewall that the rule is active.

This keeps the proxy logs clean and prevents 503 noise from unauthenticated scanners.
...
**Note on Docker Build Caching**: If you modify source code inside containers (e.g., TypeScript files), Docker's layer caching may prevent your changes from being compiled into the new image. To force a fresh rebuild:
...
The `--no-cache` flag is critical when:
- You've updated application source files (src/, lib/, etc.)
- TypeScript compilation isn't picking up changes
- Docker logs show old code behavior despite file changes

Without both `down` and `--no-cache`, Docker may reuse cached layers and your source code won't be recompiled.
...
### Reverse Proxy Environment (n8n + Cloudflare)

Applications that render full UIs (n8n, Dashboards) typically sit behind Cloudflare and nginx-proxy. They must trust proxy headers, otherwise you’ll see endless redirects or Express rate-limit warnings. Example configuration for n8n:

```yaml
environment:
  - N8N_REVERSE_PROXY=true
  - N8N_PROTOCOL=https
  - N8N_HOST=n8n.bestviable.com
  - N8N_EDITOR_BASE_URL=https://n8n.bestviable.com
  - N8N_API_BASE_URL=https://n8n.bestviable.com/api
...
|---------|------------|-----|
| Browser shows `ERR_TOO_MANY_REDIRECTS` | App thinks it is on HTTP and keeps redirecting to HTTPS | Set `*_PROTOCOL/BASE_URL` vars and restart container |
| n8n log: `ERR_ERL_UNEXPECTED_X_FORWARDED_FOR` | Express refuses proxy headers | Define `N8N_TRUSTED_PROXIES` as a JSON array |

> After changing these variables via CLI or compose, restart the service **and** nginx-proxy so the new upstream is registered: `docker compose -f infra/n8n/docker-compose.yml restart n8n` followed by `docker compose -f infra/n8n/docker-compose.yml up -d`.
...
# Confirm ACME challenge files were written during issuance
ssh tools-droplet-agents "docker exec nginx-proxy ls /usr/share/nginx/html/.well-known/acme-challenge"
```


### Error Pattern: Webhook returns 404 after workflow import (n8n)

**What you see:**

```bash
curl -s -I https://n8n.bestviable.com/webhook/memory/assemble
# HTTP/2 404
# Body: The requested webhook "POST memory/assemble" is not registered.
```
...
| Cause | Detection | Fix |
|-------|-----------|-----|
| Workflow imported/activated via CLI but n8n not restarted | `docker exec postgres psql -U n8n -d n8ndb -c 'select "webhookPath","workflowId" from "webhook_entity";'` returns 0 rows | `docker compose -f /root/portfolio/infra/n8n/docker-compose.yml restart n8n` |
| Multiple versions active (duplicate IDs) | `select "id","active" from "workflow_entity" where "name"='memory-assemble';` shows multiple `true` rows | Deactivate old versions (UI or SQL) and restart n8n |
| Wrong webhook path defined | Compare `webhook_entity.webhookPath` to desired URL | Update Webhook node path, re-import, restart |

**Default recovery steps:**
```bash
# 1. Inspect registrations
docker exec postgres psql -U n8n -d n8ndb -c 'select "webhookPath","workflowId" from "webhook_entity";'

# 2. Restart n8n to re-register production webhooks
docker compose -f /root/portfolio/infra/n8n/docker-compose.yml restart n8n

# 3. Re-test the endpoint
curl -s -I https://n8n.bestviable.com/webhook/<path>
```
...
### n8n Workflow Management Cheatsheet

| Task | Command |
|------|---------|
| Import workflow from JSON | `docker exec n8n n8n import:workflow --input=/tmp/workflow.json` |
| List workflows (IDs + names) | `docker exec n8n n8n list:workflow` |
| Activate/deactivate workflow | `docker exec n8n n8n update:workflow --id <ID> --active=true|false` (restart required for webhooks) |
| Remove duplicate registrations | `docker exec postgres psql -U n8n -d n8ndb -c 'delete from "webhook_entity";'` then restart n8n |
| Inspect webhook registry | `docker exec postgres psql -U n8n -d n8ndb -c 'select "webhookPath","workflowId" from "webhook_entity";'` |

> Always restart n8n after CLI imports/activations (`docker compose -f /root/portfolio/infra/n8n/docker-compose.yml restart n8n`). Production webhooks are only registered on startup.
...
## Docker Build Caching - Critical Issue & Resolution

### The Problem

When you update source code in a service (especially TypeScript, compiled languages), Docker's build layer caching can prevent your changes from being recompiled into the new image. This results in running old code despite making source changes.

**Symptoms**:
- Docker logs show old code behavior
- Updated TypeScript files not being compiled
- Changes don't appear in running container

### The Solution

Always use this sequence when rebuilding after code changes:

```bash
cd /path/to/service
docker-compose down         # Remove container completely
docker-compose build --no-cache     # Force fresh build without cache
docker-compose up -d        # Start new container
```

**Why both steps are critical**:

1. **`docker-compose down`**: Removes the old container completely
2. **`docker-compose build --no-cache`**: Tells Docker to:
   - Skip cached layers
   - Recompile source code (TypeScript → JavaScript, etc.)
   - Include all file changes in new image
3. **`docker-compose up -d`**: Starts fresh container with new code

**Without both commands**: Docker reuses cached layers → old compiled code runs → changes appear invisible

### Real-World Example

This solved the Coda MCP server issue when inlining MCP handler code:

```bash
ssh tools-droplet-agents
cd /root/portfolio/integrations/mcp/servers/coda

# Without --no-cache (WRONG - old code runs)
docker-compose down
docker-compose build          # Uses cached layers
docker-compose up -d          # Runs old compiled code

# Correct sequence
docker-compose down
docker-compose build --no-cache     # Forces recompilation
docker-compose up -d
docker logs coda-mcp -f             # Monitor startup
```

### When to Use `--no-cache`

Always use it when:
- ✅ Modifying application source files (src/, lib/, etc.)
- ✅ TypeScript/compiled code changes
- ✅ Debugging build issues
- ✅ After changes don't appear in running container

Skip it when:
- ❌ Only changing environment variables (no recompilation needed)
- ❌ Only changing docker-compose.yml configuration
- ❌ Pulling pre-built images from registries
