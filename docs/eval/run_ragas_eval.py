...
OUT_JSON = os.path.join(REPO_ROOT, "eval", f"eval_results_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json")
...
def load_examples(csv_path):
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for r in rdr:
            ...
            #  - runner schema: id,question,answer,expected,context
            #  - your schema :  id,question,expected_answer,expected_citations,notes
            q  = r.get("question","")
            ans = r.get("answer", "") or r.get("expected_answer","")  # fallback so you pass today
            exp = r.get("expected","") or r.get("expected_answer","")
            ctx = r.get("context","")  or r.get("expected_citations","")
            rid = r.get("id") or q[:40]
            rows.append({"id": rid, "question": q, "answer": ans, "expected": exp, "context": ctx})
    return rows
