...
exports.createSimpleAuthMiddleware = createSimpleAuthMiddleware;
const cloudflare_access_1 = require("../validators/cloudflare-access");
const bearer_token_1 = require("../validators/bearer-token");
...
function createAuthMiddleware(config, options = {}) {
    const { onSuccess, onError, getServiceToken } = options;
        ...
        try {
            ...
            if (cfAccessJWT && (config.mode === 'cloudflare' || config.mode === 'both')) {
                try {
                    ...
                    const userInfo = await (0, cloudflare_access_1.validateCloudflareAccessJWT)(cfAccessJWT, config.cloudflareAccessTeamDomain, config.cloudflareAccessAud);
                    authenticated = true;
            ...
            if (!authenticated && authHeader && (config.mode === 'bearer' || config.mode === 'both')) {
                ...
                const validation = (0, bearer_token_1.validateBearerToken)(authHeader, bearerConfig);
                if (validation.valid) {
                    authenticated = true;
 ...
 */
function createSimpleAuthMiddleware(config) {
    return createAuthMiddleware(config, {
        getServiceToken: async (_req) => {
