...
/**
 * Verify token with Firecrawl API
 *
 * Validates bearer token by calling Firecrawl's health endpoint
 */
export async function verifyToken(token: string): Promise<AuthInfo> {
  try {
    const response = await fetch('https://api.firecrawl.dev/health', {
      method: 'GET',
      headers: {
